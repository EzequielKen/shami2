...
namespace _03___sistemas_fabrica
{
    ...
    public class cls_estadisticas_de_entrega
    {
        ...
        private void calcular_estadisticas_segun_sucursal()
        {
            ...
            for (int fila = 0; fila <= pedido_de_insumos.Rows.Count - 1; fila++)
            {
                columna = pedido_de_insumos.Columns["producto_1"].Ordinal;
                while (columna <= pedido_de_insumos.Columns.Count - 1 &&
                pedido_de_insumos.Rows[fila][columna].ToString() != "N/A")
                {
                    id_producto = funciones.obtener_dato(pedido_de_insumos.Rows[fila][columna].ToString(), 1);
                    cargar_estadistica_produccion(fila, columna, id_producto, insumos_fabrica, "insumos_fabrica", pedido_de_insumos, 3);
                    columna++;
                }
            }
            string id;
            ...
            int fila_producto;
            double cantidad_pedida;
            for (int filas = ante_ultima_fila + 1; filas <= resumen.Rows.Count - 1; filas++)
            {
        ...
        private void consultar_pedidos_segun_sucursal(DataTable resumen_sucursales, string fecha_inicio, string fecha_fin)
        {
            ...
            {
                lista_pedidos_sucursales.Add(consultas.consultar_pedidos_segun_rango_de_fecha(resumen_sucursales.Rows[fila]["sucursal"].ToString(), fecha_inicio, fecha_fin));
            }
        }
