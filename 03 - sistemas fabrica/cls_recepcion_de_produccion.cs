...
{
    [Serializable]
    public class cls_recepcion_de_produccion
    {
        public cls_recepcion_de_produccion(DataTable usuario_BD)
        {
        ...
        public void confirmar_produccion(string id_historial, DataTable detalle_produccion, string proveedor, string rol_usuario)
        {
            ...
            for (int fila = 0; fila <= detalle_produccion.Rows.Count - 1; fila++)
            {
                ...
                consultas.actualizar_tabla(base_de_datos, "produccion_diaria", actualizar, id_historial);
            }
            consultas.actualizar_tabla(base_de_datos, "produccion_diaria", "`estado` = 'Recibido' ", id_historial);
