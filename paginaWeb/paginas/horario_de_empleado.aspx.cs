...
{
    public partial class horario_de_empleado : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                horarios_de_empleados = planificador.get_horarios_de_empleados(sucursal.Rows[0]["id"].ToString(), fecha_inicio, fecha_fin);
                Session["horarios_de_empleados"] = horarios_de_empleados;
                empleados = planificador.get_empleado(empleado.Rows[0]["id"].ToString());
        ...
        protected void gridview_empleados_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                for (int columna_index = 3; columna_index <= gridview_empleados.Columns.Count - 1; columna_index++)
                {
                    ...
                    e.Row.Cells[columna_index].Controls.Add(labelEntrada);
                    e.Row.Cells[columna_index].Controls.Add(new Literal { Text = "<br />Salida: " });
                    e.Row.Cells[columna_index].Controls.Add(labelSalida);
            ...
            for (int fila = 0; fila <= gridview_empleados.Rows.Count - 1; fila++)
            {
                ...
                for (int columna = 3; columna <= gridview_empleados.Columns.Count - 1; columna++)
                {
                    ...
                    {
                        Label label_entrada = (gridview_empleados.Rows[fila].Cells[columna].FindControl($"{fecha.ToString("dd-MM-yyyy")}-entrada-{id_empleado}") as Label);
                        Label label_salida = (gridview_empleados.Rows[fila].Cells[columna].FindControl($"{fecha.ToString("dd-MM-yyyy")}-salida-{id_empleado}") as Label);
                        ...
                        if (funciones.IsNotDBNull(horarios_de_empleados.Rows[fila_horario]["horario_entrada"]))
                        {
                            ...
                            label_entrada.Text = hora_entrada.ToString("HH:mm");
                        }
                        if (funciones.IsNotDBNull(horarios_de_empleados.Rows[fila_horario]["horario_salida"]))
                        {
                            ...
                            label_salida.Text = hora_salida.ToString("HH:mm");
                        }
                        if (horarios_de_empleados.Rows[fila_horario]["franco"].ToString() == "Si")
                        {
