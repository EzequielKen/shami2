...
{
    public partial class alta_de_empleados : System.Web.UI.Page
    {
        ...
        private void crear_lista_de_empleado()
        {
            ...
            lista_de_empleado.Columns.Add("sueldo", typeof(string));
            lista_de_empleado.Columns.Add("contraseña", typeof(string));
        ...
        private void llenar_tabla_empleado()
        {
            ...
            for (int fila = 0; fila <= lista_de_empleadoBD.Rows.Count - 1; fila++)
            {
                    ...
                    dropDown_tipo.SelectedItem.Text == "Todos")
                {
                    ...
                    lista_de_empleado.Rows[ultima_fila]["sueldo"] = lista_de_empleadoBD.Rows[fila]["sueldo"].ToString();
                    lista_de_empleado.Rows[ultima_fila]["contraseña"] = lista_de_empleadoBD.Rows[fila]["contraseña"].ToString();
        ...
        private void crear_tabla_empleado()
        {
            ...
            empleado.Columns.Add("sueldo", typeof(string));
        }
        private void cargar_datos_empleado()
        {
            if (verificar_carga())
            {
                ...
                empleado.Rows[0]["sueldo"] = textbox_sueldo.Text;
                ...
                else
                {
                    ...
                    textbox_sueldo.Text = string.Empty;
        ...
        private bool verificar_carga()
        {
            ...
            if (textbox_sueldo.Text == string.Empty)
            {
                mensaje = mensaje + "Falta ingresar sueldo. ";
            }
            ...
            if (cantidad_cargos > 1)
            {
                mensaje = mensaje + "Asigne 1 solo cargo. ";
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            lista_de_empleadoBD = administrador.get_lista_de_empleado_origen(usuariosBD.Rows[0]["sucursal"].ToString(), fecha_de_hoy);
            if (!IsPostBack)
            {
                configurar_cargos();
                configurar_controles();
        ...
        protected void textbox_sueldo_empleado_TextChanged(object sender, EventArgs e)
        {
            TextBox textbox_sueldo_empleado = (TextBox)sender;
            GridViewRow row = (GridViewRow)textbox_sueldo_empleado.NamingContainer;
            int rowIndex = row.RowIndex;
            string id = gridview_empleados.Rows[rowIndex].Cells[0].Text;
            string campo = "sueldo";
            string dato = textbox_sueldo_empleado.Text;
            double cantidad;
            if (double.TryParse(dato, out cantidad))
            {
                administrador.actualizar_dato_empleado(id, campo, cantidad.ToString());
            }
            lista_de_empleadoBD = administrador.get_lista_de_empleado(usuariosBD.Rows[0]["sucursal"].ToString(), fecha_de_hoy);
            configurar_controles();
            cargar_lista_empleados();
        ...
        protected void gridview_empleados_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            for (int fila = 0; fila <= gridview_empleados.Rows.Count - 1; fila++)
            {
                ...
                TextBox textbox_contraseña_empleado = (gridview_empleados.Rows[fila].Cells[0].FindControl("textbox_contraseña_empleado") as TextBox);
                TextBox textbox_telefono_empleado = (gridview_empleados.Rows[fila].Cells[0].FindControl("textbox_telefono_empleado") as TextBox);
                TextBox textbox_sueldo_empleado = (gridview_empleados.Rows[fila].Cells[0].FindControl("textbox_sueldo_empleado") as TextBox);
                Label label_sueldo = (gridview_empleados.Rows[fila].Cells[0].FindControl("label_sueldo") as Label);
                ...
                textbox_contraseña_empleado.Text = lista_de_empleado.Rows[fila]["contraseña"].ToString();
                textbox_telefono_empleado.Text = lista_de_empleado.Rows[fila]["telefono"].ToString();
                textbox_sueldo_empleado.Text = lista_de_empleado.Rows[fila]["sueldo"].ToString();
                label_sueldo.Text = funciones.formatCurrency(double.Parse(lista_de_empleado.Rows[fila]["sueldo"].ToString()));
        ...
        protected void textbox_sueldo_TextChanged(object sender, EventArgs e)
        {
            double sueldo;
            if (double.TryParse(textbox_sueldo.Text, out sueldo))
            {
                label_ingreso_sueldo.Text = funciones.formatCurrency(sueldo);
            }
            else
            {
                textbox_sueldo.Text = string.Empty;
        ...
        protected void textbox_contraseña_empleado_TextChanged(object sender, EventArgs e)
        {
            TextBox textbox_dni_empleado = (TextBox)sender;
            GridViewRow row = (GridViewRow)textbox_dni_empleado.NamingContainer;
            int rowIndex = row.RowIndex;
