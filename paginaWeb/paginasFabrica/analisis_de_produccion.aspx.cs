...
{
    public partial class analisis_de_produccion1 : System.Web.UI.Page
    {
        #region cargar tabla
        private void crear_tabla_resumen()
        {
            ...
            resumen.Columns.Add("cantidad_producida", typeof(string));
            resumen.Columns.Add("cantidad_recibida", typeof(string));
            resumen.Columns.Add("unidad_de_medida_produccion", typeof(string));
        }
        private void llenar_tabla_resumen()
        {
            ...
            for (int fila = 0; fila <= estadisticas_de_pedidos_seleccionados.Rows.Count - 1; fila++)
            {
                if (funciones.verificar_tipo_producto(estadisticas_de_pedidos_seleccionados.Rows[fila]["tipo_producto"].ToString(), dropDown_tipo.SelectedItem.Text))
                {
                    ...
                    resumen.Rows[fila_resumen]["cantidad_producida"] = estadisticas_de_pedidos_seleccionados.Rows[fila]["cantidad_producida"].ToString();
                    resumen.Rows[fila_resumen]["cantidad_recibida"] = estadisticas_de_pedidos_seleccionados.Rows[fila]["cantidad_recibida"].ToString();
                    resumen.Rows[fila_resumen]["unidad_de_medida_produccion"] = estadisticas_de_pedidos_seleccionados.Rows[fila]["unidad_de_medida_produccion"].ToString();
                }
            }
        }
        ...
        /// </summary>
        #region atributos
        cls_analisis_de_produccion analisis;
        ...
        protected void boton_calcular_Click(object sender, EventArgs e)
        {
                ...
                Session["fecha_estadistica_fin"].ToString() != "N/A")
            {
                ...
                estadisticas_de_pedidos_seleccionados = analisis.get_analisis_produccion(Session["fecha_estadistica_inicio"].ToString(), Session["fecha_estadistica_fin"].ToString());
