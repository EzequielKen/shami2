...
{
    public partial class pedir_insumos : System.Web.UI.Page
    {
        ...
        private void llenar_tabla_insumos()
        {
            ...
            for (int fila = 0; fila <= insumos_proveedorBD.Rows.Count - 1; fila++)
            {
                    ...
                    funciones.buscar_alguna_coincidencia(textbox_busqueda.Text, insumos_proveedorBD.Rows[fila]["producto"].ToString()))
                {
                    ...
                    //   insumos_proveedor.Rows[fila_insumo]["precio"] = funciones.formatCurrency(double.Parse(insumos_proveedorBD.Rows[fila]["precio"].ToString()));

                    fila_insumo++;
                }
            }
        }
        private void llenar_tabla_insumos_busqueda()
        {
            crear_tabla_insumos();
            int fila_insumo = 0;
            for (int fila = 0; fila <= insumos_proveedorBD.Rows.Count - 1; fila++)
            {
                if (funciones.buscar_alguna_coincidencia(textbox_busqueda.Text, insumos_proveedorBD.Rows[fila]["producto"].ToString()))
                {
        ...
        private void cargar_insumos_busqueda()
        {
            tipo_seleccionado = dropDown_tipo.SelectedItem.Text;
            Session.Add("tipo_seleccionado", dropDown_tipo.SelectedItem.Text);


            llenar_tabla_insumos_busqueda();
        ...
        private void cargar_producto_en_resumen(string cantidad_dato, string precio)
        {
            ...
            producto = gridview_productos.SelectedRow.Cells[1].Text;
            ...
            while (fila <= insumos_proveedorBD.Rows.Count - 1)
            {
                if (id_producto == insumos_proveedorBD.Rows[fila]["id"].ToString() &&
                    producto == insumos_proveedorBD.Rows[fila]["producto"].ToString() &&
                    !verificar_sicargo(id_producto, producto))
                {
        ...
        private bool verificar_sicargo(string id_producto,string producto)
        {
            bool retorno = false;
            int fila = 0;
        ...
        /// //////////////////////////////////////////////////////////////////////////
        /// </summary>
        cls_pedir_insumos pedir_insumo;
        ...
        protected void boton_enviar_Click(object sender, EventArgs e)
        {
            ...
            if (resumen_pedido.Rows.Count > 0)
            {
                ...
                string url_whatsapp = pedir_insumo.cargar_pedido_de_insumos(resumen_pedido, (DataTable)Session["tipo_usuario"]);
