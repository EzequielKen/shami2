...
{
    public class cls_evaluar_visita_operativa
    {
        public cls_evaluar_visita_operativa(DataTable usuario_BD)
        {
        ...
        public void actualizar_nota(string id, string nota)
        {
            ...
            consultas.actualizar_tabla(base_de_datos, "historial_evaluacion_chequeo", actualizar, id);
        }
        public void actualizar_punto_real(string id, string punto_real)
        {
            string actualizar = "`punto_real` = '" + punto_real + "' ";
            consultas.actualizar_tabla(base_de_datos, "historial_evaluacion_chequeo", actualizar, id);
        }
        #endregion
        ...
        private void crear_tabla_historial_resumen()
        {
            ...
            historial_resumen.Columns.Add("punto_teorico", typeof(string));
            historial_resumen.Columns.Add("punto_real", typeof(string));
        ...
        private void llenar_historial_resumen()
        {
            ...
            for (int fila = 0; fila <= historial.Rows.Count - 1; fila++)
            {
                ...
                historial_resumen.Rows[ultima_fila]["punto_teorico"] = historial.Rows[fila]["punto_teorico"].ToString();
                historial_resumen.Rows[ultima_fila]["punto_real"] = historial.Rows[fila]["punto_real"].ToString();
            }
        }
